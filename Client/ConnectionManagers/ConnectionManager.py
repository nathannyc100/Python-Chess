...
class ConnectionManager(threading.Thread, QObject):
    ...
    def __init__(self, isServer: bool, sharedResources: SharedResources.SharedResources):
        ...
        if self.isServer:
            ...
            self.receiveThread = ReceiveThread(self, sharedResources)
        #end
...
class ReceiveThread(threading.Thread, QObject):
    ...
    def __init__(self, connectionManager: ConnectionManager, sharedResources: SharedResources.SharedResources):
        threading.Thread.__init__(self)
        QObject.__init__(self)
            ...
            if manageSocket:
                try:
                    ...
                    if self.connectionManager.isServer == False:
                        self.ServerDisconnect(manageSocket)
                    #end
                #end
            #end
        #end
    ...
    def ServerDisconnect(self, manageSocket: socket.socket):
        self.connectionManager.tcpSocket = None
        manageSocket.close()
        self.sharedResources.serverDisconnectSignal.emit()
    #end
